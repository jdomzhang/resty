# Disclamation
<h2>
This fork is to skip the annoying error when using resty with `go mod`, and only for experimental purpose.
</h2>
<hr/>
...
## Features
  ...
  * [Response](https://godoc.org/github.com/jdomzhang/resty#Response) object gives you more possibility
    * Access as `[]byte` array - `response.Body()` OR Access as `string` - `response.String()`
    * Know your `response.Time()` and when we `response.ReceivedAt()`
  * Automatic marshal and unmarshal for `JSON` and `XML` content type
    * Default is `JSON`, if you supply `struct/map` without header `Content-Type`
    * For auto-unmarshal, refer to -
        - Success scenario [Request.SetResult()](https://godoc.org/github.com/jdomzhang/resty#Request.SetResult) and [Response.Result()](https://godoc.org/github.com/jdomzhang/resty#Response.Result).
        - Error scenario [Request.SetError()](https://godoc.org/github.com/jdomzhang/resty#Request.SetError) and [Response.Error()](https://godoc.org/github.com/jdomzhang/resty#Response.Error).
        - Supports [RFC7807](https://tools.ietf.org/html/rfc7807) - `application/problem+json` & `application/problem+xml`
  * Easy to upload one or more file(s) via `multipart/form-data`
    * Auto detects file content type
  * Request URL [Path Params (aka URI Params)](https://godoc.org/github.com/jdomzhang/resty#Request.SetPathParams)
  * Backoff Retry Mechanism with retry condition function [reference](retry_test.go)
  * resty client HTTP & REST [Request](https://godoc.org/github.com/jdomzhang/resty#Client.OnBeforeRequest) and [Response](https://godoc.org/github.com/jdomzhang/resty#Client.OnAfterResponse) middlewares
  ...
  * Custom [Root Certificates](https://godoc.org/github.com/jdomzhang/resty#Client.SetRootCertificate) and Client [Certificates](https://godoc.org/github.com/jdomzhang/resty#Client.SetCertificates)
  * Download/Save HTTP response directly into File, like `curl -o` flag. See [SetOutputDirectory](https://godoc.org/github.com/jdomzhang/resty#Client.SetOutputDirectory) & [SetOutput](https://godoc.org/github.com/jdomzhang/resty#Request.SetOutput).
  * Cookies for your request and CookieJar support
  * SRV Record based request instead of Host URL
  * Client settings like `Timeout`, `RedirectPolicy`, `Proxy`, `TLSClientConfig`, `Transport`, etc.
  * Optionally allows GET request with payload, see [SetAllowGetMethodPayload](https://godoc.org/github.com/jdomzhang/resty#Client.SetAllowGetMethodPayload)
  ...
  * Resty design
    ...
    * Supports `http.RoundTripper` implementation, see [SetTransport](https://godoc.org/github.com/jdomzhang/resty#Client.SetTransport)
...
```bash
require github.com/jdomzhang/resty v1.10.1
```

##### go get
```bash
go get -u github.com/jdomzhang/resty
```
...
```go
import "github.com/jdomzhang/resty"
```
